...
> - 容器启动后，即可通过访问 `http://服务器IP:3000` 畅玩运行！
> - 若需要修改映射端口（默认 3000），进入 `docker-compose.yml`，修改 `ports:` 下的 `- "3000:3000"`，比如 `- "80:3000"` 就可以直接通过 80 端口无需反向代理访问。
...
server {
    ...
    location / {
        # 转发到容器内部的 Express 服务。
        # 注意：proxy_pass 始终指向 127.0.0.1:3000，因为容器内部 Express 固定监听 3000。
        # 宿主机端口映射由 docker-compose.yml 的 ports (${APP_PORT}:3000) 控制，无需在此修改。
        # 如果通过 docker-compose 暴露了非 80/443 端口，也可省略 Nginx，直接访问 http://IP:APP_PORT。
