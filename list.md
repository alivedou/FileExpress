...
### 7. Docker 容器基础镜像 Node.js 运行时环境升级 [已完成]
- [x] **升级底层镜像运行时至当代 Active LTS 黄金标准 (`Node 22`)**:
  - 1️⃣ **原理剖析与版本抉择**：
    - **Node 20** 正处于 Maintenance（维护/生命周期末期）状态，安全补丁响应速度降低。
    - **Node 22 (目前最主流的 Active LTS，长期支持版)**：引入了 V8 引擎针对 ESM/CJS 加载的革命性优化、原生的 WebSocket 内置原生客户端、以及极大地提升了文件系统 I/O (File System) 效率及内存回收调度。这是生产容器化部署当之无愧的黄金主流和必然归宿。
    - **Node 24 (暂非 active LTS)**：属于最新的试验/活跃开发大分支，极少数底层 node_modules 编译链（如部分 Alpine native-binding 的二进制依赖包）可能会面临不向下兼容的阵痛。
    - **深度决策**：生产环境应当采用最具高可用及前瞻性的 **`node:22-alpine`** 单层轻量基础镜像作为最终极速运行底座。
  - 2️⃣ **具体执行细节**：
    - 替换 `/Dockerfile` 第 4 行（构建阶段）和第 27 行（运行阶段）使用的 `node:20-alpine` 运行时，统一冷升级变更为目前完美契合生产环境的 `node:22-alpine`。
    - 在 GitHub Actions 跨平台容器极速打包构建流水线（`.github/workflows/docker-publish.yml`）中全程利用 Buildx 编译并分发适用于 `linux/amd64` 与 `linux/arm64` 极速架构上的 Node 22 分发包。
- [x] **清理历史遗留暴露端口 (将 3456 彻底统一为 3000)**:
  - 1️⃣ 统一在 `/fe.sh` 的应用配置面板、输入引导命令、部署提示语以及服务器状态回显中将旧的默认推荐外部端口 `3456` 全盘自愈替换为统一的默认端口 `3000`。
...
### 1. 修改位置
- `list.md` (增量追加待办任务)
- `Dockerfile` (将 `node:20-alpine` 替换为最新主流 Active LTS `node:22-alpine`，待用户授权同意后实施)

### 2. 影响范围
- **业务代码**：**完全无影响**。因为业务代码并不使用任何依赖于 Node 底层特定特定非标准 C/C++ 动态原生插件，所以冷切换运行时绝对丝滑。
- **本地宿主**：无影响。开发者和生产部署仅会在下一次使用 `fe.sh`（或 `docker compose build`）时重新拉取轻量级的 `node:22-alpine` 镜像并全盘缓存在本地。
- **CI/CD**：全新拉起的 GitHub Actions 自动化编译流会自动以 Node 22 模块镜像打包新 Tag 生产分发包。

### 3. 风险等级
- **风险等级**：🟢 **极低风险 (Almost Zero Risk)**。
  - 属于在安全稳妥重塑后的运行时版本平滑迭代，Node 22 向下全方位完美兼容 Node 20 语法生态，不存在任何技术断层。
